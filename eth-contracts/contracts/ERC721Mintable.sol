...
contract Ownable {
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        ...
        _owner = newOwner;
        emit TransferOwner(previousOwner, newOwner);
    }
...
//  2) create a public setter using the inherited onlyOwner modifier 
    function setPaused(bool paused) public onlyOwner {
        require(paused != _paused, "Paused is already set to that state");
        _paused = paused;
        if (_paused) {
            emit Paused(owner());
        } else {
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(ownerOf(tokenId) != to, "Address is already owner");
        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == ownerOf(tokenId) || isApprovedForAll(ownerOf(tokenId), msg.sender), "Caller must be owner or approved for all");
    ...
    function getApproved(uint256 tokenId) public view returns (address) {
        ...
        require(_tokenApprovals[tokenId] != address(0), "no approvals for token");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(_tokenOwner[tokenId] == from, "Address is not token owner");
...
contract ERC721MintableComplete is ERC721Metadata {
    constructor() public ERC721Metadata("CapstoneERC721Token", "CAPST", "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/"){
    }
